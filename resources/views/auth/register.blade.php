...
@section('titulo', 'Cadastro Coleta')
...
@section('conteudo')
    ...
    <div class="container">
        ...
        <form class="form-signin" form method="POST" action="{{ route('register') }}">
            @csrf
            <h2 class="form-signin-heading">Registro Coleta</h2>
            <div class="login-wrap">
                <input id="name" type="text" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" value="{{ old('name') }}" placeholder="Nome">
                @if ($errors->has('name'))
                <span class="invalid-feedback" role="alert">
                    <strong style="color: red"><i class="fa fa-exclamation-circle"></i> {{ $errors->first('name') }}</strong>
                </span>
                @endif
